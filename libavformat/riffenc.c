                      ...
                      AVCodecParameters *par, int flags)
{
    ...
    if (par->codec_id == AV_CODEC_ID_MP2) {
        ...
        if (par->libmp3lame_cbr == 1) blkalign = 1;
                              ...
                              int *au_ssize, int *au_scale)
{
    ...
    *au_scale /= gcd;
    *au_rate  /= gcd;
    if (par->codec_type == AVMEDIA_TYPE_AUDIO && par->codec_id == AV_CODEC_ID_MP3 && par->libmp3lame_cbr == 1) {
        par->block_align = 1;
        *au_ssize = 1; /* dwSampleSize */
        *au_rate = par->bit_rate / 8; /* dwRate */
        *au_scale = 1; /* dwScale */
    }
}
