...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (trk->entry) {
        ...
        if (duration < 0 || duration > INT_MAX) {
            av_log(s, AV_LOG_WARNING, "Application provided duration: %"PRId64" / timestamp: %"PRId64" is out of range for mov/mp4 format\n",
                duration, pkt->dts
            );
        ...
        if (pkt->duration < 0) {
            av_log(s, AV_LOG_WARNING, "Application provided duration: %d is invalid\n", pkt->duration);
        }
    }
    if (mov->flags & FF_MOV_FLAG_FRAGMENT) {
        int ret;
...
AVOutputFormat ff_mov_muxer = {
    ...
    .flags             = AVFMT_GLOBALHEADER | AVFMT_ALLOW_FLUSH | AVFMT_TS_NEGATIVE | AVFMT_VARIABLE_FPS,
...
AVOutputFormat ff_f4v_muxer = {
    ...
    .write_packet      = mov_write_packet,
    .write_trailer     = mov_write_trailer,
    .flags             = AVFMT_GLOBALHEADER | AVFMT_ALLOW_FLUSH | AVFMT_VARIABLE_FPS,
