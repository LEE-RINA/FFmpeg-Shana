...
static void dump_metadata(void *ctx, AVDictionary *m, const char *indent, int hide_msg)
{
                               ...
                               int index, int is_output, int hide_msg)
{
    ...
    if (!hide_msg)
    {
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (fps || tbr || tbn || tbc)
                if (!hide_msg) av_log(NULL, AV_LOG_INFO, "%s", separator);
                    ...
                    const char *url, int is_output, int hide_msg)
{
    ...
    if (!is_output) {
        ...
        if (ic->duration != AV_NOPTS_VALUE) {
            ...
            hours = mins / 60;
            mins %= 60;
            if (!hide_msg) av_log(NULL, AV_LOG_INFO, "%02d:%02d:%02d.%02d", hours, mins, secs,
                   (100 * us) / AV_TIME_BASE);
        } else {
        ...
        if (ic->start_time != AV_NOPTS_VALUE) {
            ...
            if (!hide_msg) av_log(NULL, AV_LOG_INFO, "%d.%06d",
                   secs, (int) av_rescale(us, 1000000, AV_TIME_BASE));
        }
        if (!hide_msg) av_log(NULL, AV_LOG_INFO, ", bitrate: ");
        if (ic->bit_rate)
            if (!hide_msg) av_log(NULL, AV_LOG_INFO, "%d kb/s", ic->bit_rate / 1000);
        else
            if (!hide_msg) av_log(NULL, AV_LOG_INFO, "N/A");
    ...
    if (ic->nb_programs) {
        ...
        for (j = 0; j < ic->nb_programs; j++) {
            ...
            if (!hide_msg) av_log(NULL, AV_LOG_INFO, "  Program %d %s\n", ic->programs[j]->id,
                   name ? name->value : "");
            dump_metadata(NULL, ic->programs[j]->metadata, "    ", hide_msg);
