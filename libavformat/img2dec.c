...
int ff_img_read_header(AVFormatContext *s1)
{
    ...
    MultiByteToWideChar(CP_UTF8, 0, s->path, -1, path_wchar, 1024);
    WideCharToMultiByte(CP_THREAD_ACP, 0, path_wchar, -1, path_char, 1024, NULL, NULL);
    ...
    if (!s->is_pipe) {
        ...
        s->use_glob = is_glob(path_char);
        if (s->use_glob) {
#if HAVE_GLOB
            char *p = path_char, *q, *dup;
            int gerr;
#endif
            ...
            while (*q) {
                ...
                if ((p - path_char) >= (sizeof(path_char) - 2))
                  break;
                if (*q == '%' && strspn(q + 1, "%*?[]{}"))
                    ++q;
                else if (strspn(q, "\\*?[]{}"))
            ...
            *p = 0;
            ...
            gerr = glob(path_char, GLOB_NOCHECK|GLOB_BRACE|GLOB_NOMAGIC, NULL, &s->globstate);
            if (gerr != 0) {
        ...
        if ((s->pattern_type == PT_GLOB_SEQUENCE && !s->use_glob) || s->pattern_type == PT_SEQUENCE) {
            if (find_image_range(s1->pb, &first_index, &last_index, path_char,
                                 s->start_number, s->start_number_range) < 0) {
                av_log(s1, AV_LOG_ERROR,
                       "Could find no file with path '%s' and index in the range %d-%d\n",
                       path_char, s->start_number, s->start_number + s->start_number_range - 1);
                return AVERROR(ENOENT);
            }
    ...
    if (s1->video_codec_id) {
        ...
        const char *str = strrchr(path_char, '.');
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
            st->codecpar->codec_id = ff_guess_image2_codec(path_char);
