...
static av_cold int mp3lame_encode_init(AVCodecContext *avctx)
{
    ...
    if (s->lamechmode == 0) {
        //STEREO
        lame_set_mode(s->gfp, STEREO);
    } else if (s->lamechmode == 1) {
        //JOINT_STEREO
        lame_set_mode(s->gfp, JOINT_STEREO);
    } else if (s->lamechmode == 2) {
        //DUAL_CHANNEL
        lame_set_mode(s->gfp, DUAL_CHANNEL);
    } else if (s->lamechmode == 3) {
        //MONO
        lame_set_mode(s->gfp, MONO);
    } else if (s->lamechmode == -1) {
        //AUTO
        lame_set_mode(s->gfp, avctx->ch_layout.nb_channels > 1 ? JOINT_STEREO : MONO);
    }
    ...
    /* algorithmic quality :: Lame Option -q */
    if (avctx->compression_level == FF_COMPRESSION_DEFAULT)
        if (s->lamebtmode == -1 || s->lamebtmode == 0 || s->lamebtmode == 1) 
            lame_set_quality(s->gfp, 5);
        else
            lame_set_quality(s->gfp, 0);
    else
    ...
    } else if (s->lamebtmode == 2) {
        //VBR vbr-old
        lame_set_VBR(s->gfp, vbr_rh);
        lame_set_VBR_quality(s->gfp, avctx->global_quality / (float)FF_QP2LAMBDA);

    } else if (s->lamebtmode == 3) {
        //VBR vbr-new
        lame_set_VBR(s->gfp, vbr_mt);
        lame_set_VBR_quality(s->gfp, avctx->global_quality / (float)FF_QP2LAMBDA);

    } else if (s->lamebtmode == 4) {
        //VBR vbr-mtrh
        lame_set_VBR(s->gfp, vbr_mtrh);
        lame_set_VBR_quality(s->gfp, avctx->global_quality / (float)FF_QP2LAMBDA);

    } else if (s->lamebtmode == 5) {
        //VBR vbr_default
...
static const AVOption options[] = {
    ...
    { "abr",          NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 1 },                       INT_MIN, INT_MAX, AE, "lamebtmode" },
    { "vbr(rh)",      NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 2 },                       INT_MIN, INT_MAX, AE, "lamebtmode" },
    { "vbr(mt)",      NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 3 },                       INT_MIN, INT_MAX, AE, "lamebtmode" },
    { "vbr(mtrh)",    NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 4 },                       INT_MIN, INT_MAX, AE, "lamebtmode" },
    { "vbr(default)", NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 5 },                       INT_MIN, INT_MAX, AE, "lamebtmode" },
    { "lamechmode",  "Set channel mode", OFFSET(lamechmode), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, 3, AE, "lamechmode"},
    { "auto",         NULL,         0, AV_OPT_TYPE_CONST, { .i64 = -1 },                      INT_MIN, INT_MAX, AE, "lamechmode" },
    { "stereo",       NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 0 },                       INT_MIN, INT_MAX, AE, "lamechmode" },
    { "joint_stereo", NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 1 },                       INT_MIN, INT_MAX, AE, "lamechmode" },
    { "dual_channel", NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 2 },                       INT_MIN, INT_MAX, AE, "lamechmode" },
    { "mono",         NULL,         0, AV_OPT_TYPE_CONST, { .i64 = 3 },                       INT_MIN, INT_MAX, AE, "lamechmode" },
