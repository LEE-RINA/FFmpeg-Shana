...
static int create(VkRenderer *renderer, SDL_Window *window, AVDictionary *opt)
{
    ...
    /* shana */
    entry = av_dict_get(opt, "swapchain_colorspace", NULL, 0);
    if (entry && strtol(entry->value, NULL, 10))
        ctx->swapchain_colorspace = 1;
    else
        ctx->swapchain_colorspace = 0;
    /* shana */
