...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if (!oc->nb_streams && !(oc->oformat->flags & AVFMT_NOSTREAMS)) {
            av_dump_format(oc, i, oc->filename, 1, hide_msg);
...
//         assert_avoptions(output_files[i]->opts);
 ...
 dump_format:
    /* dump the file output parameters - cannot be done before in case
       of stream copy */
    for (i = 0; i < nb_output_files; i++) {
        av_dump_format(output_files[i]->ctx, i, output_files[i]->ctx->filename, 1, hide_msg);
    }
...
static int transcode(void)
{
    ...
    if (stdin_interaction && gui_hwnd == NULL) {
    ...
    while (!received_sigterm) {
        ...
        /* gui hwnd check */
            ...
            if (gui_hwnd != NULL)
            {
                HWND gui_hWnd = (HWND)(LONG_PTR)atol(gui_hwnd);
                if (!IsWindow(gui_hWnd))
                    break;
            }
        #endif
