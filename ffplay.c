...
{
    SDL_memset(q, 0, sizeof(PacketQueue));
...
static inline void fill_rectangle(int x, int y, int w, int h, int r, int g, int b, int a)
{
    if (renderer != NULL)
    {
        SDL_Rect rect;
...
static void video_image_display(VideoState *is)
{
    ...
    if (frame_text_hwnd != NULL)
    {
        ...
        SendMessage(frametext_hWnd, 0x000C, 0, (LPARAM) is->frame_info[is->pictq_rindex]);
    }
#endif
    ...
    if (renderer != NULL) 
    {
        ...
        SDL_RenderCopy(renderer, vp->bmp, NULL, &is->last_display_rect);
...
static void video_audio_display(VideoState *s)
{
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        fill_rectangle(s->xleft, s->ytop, s->width, s->height, 0, 0, 0, 0);
        ...
        for (ch = 0; ch < nb_display_channels; ch++) {
            ...
            for (x = 0; x < s->width; x++) {
                ...
                fill_rectangle(s->xleft + x, ys, 1, y, 255, 255, 255, 0);
                i += channels;
        ...
        for (ch = 1; ch < nb_display_channels; ch++) {
            y = s->ytop + ch * h;
            fill_rectangle(s->xleft, y, s->width, 1, 0, 0, 255, 0);
        }
        ...
        }
        {
            ...
            for (y = 0; y < s->height; y++) {
                ...
                a = FFMIN(a, 255);
                b = FFMIN(b, 255);
                fill_rectangle(s->xpos, s->height-y, 1, 1, a, b, (a+b)/2, 0);
            }
            if (renderer != NULL) SDL_RenderPresent(renderer);
        }
        if (!s->paused)
...
static void s_window_init(VideoPicture *vp, VideoState *is);
static void video_display(VideoState *is)
{
    if (!renderer && !window) s_window_init(NULL, is);
...
display:
    ...
    if (show_status) {
        ...
        if (!last_time || (cur_time - last_time) >= 30000) {
            ...
            //clock
            playtime = (get_master_clock(is) * AV_TIME_BASE) - starttime + 5000;
            ...
            if (playtime >= 0)
            {
                ...
                snprintf(buf_s, sizeof(buf_s), "%02d:%02d:%02d.%02d, %3.1fx, A-V:%7.3f, vol=%d(%d%%)",
                                               hours, mins, secs, (100 * us) / AV_TIME_BASE, speed_s, av_diff, volume_s, volpct);
            }
            else
            {
                snprintf(buf_s, sizeof(buf_s), "time=NaN, %3.1fx, A-V:%7.3f, vol=%d(%d%%)",
                                               speed_s, av_diff, volume_s, volpct);
            }
            ...
            if (window_text_hwnd == NULL)
            {
                printf("%7.2f A-V:%7.3f fd=%4d aq=%5dKB vq=%5dKB sq=%5dB f=%"PRId64"/%"PRId64"   %3.1fx  \r",
                get_master_clock(is),
                av_diff,
...
static void s_window_init(VideoPicture *vp, VideoState *is)
{
    if (!renderer && !window)
    {
        ...
        // 화면 사이즈
        if (screen_width) // 사용자 설정
        {
            is->width = screen_width;
            is->height = screen_height;
        }
        else if (vp != NULL && vp->width > 0 && vp->height > 0) // 비디오가 있음
        {
            is->width = vp->width;
            is->height = vp->height;
        }
...
static void alloc_picture(VideoState *is)
{
    ...
    s_window_init(vp, is);
		...
		vp->bmp = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING, w, h);
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if (frame_text_hwnd != NULL)
    {
        ...
        if (start_time_s != AV_NOPTS_VALUE) starttime = start_time_s;
        ptstime = ((float)pts * AV_TIME_BASE) - starttime + 5000;
        ...
        if (ptstime >= 0)
        {
            ...
            snprintf(is->frame_info[is->pictq_windex], sizeof(is->frame_info[is->pictq_windex]), "%02d:%02d:%02d.%02d, width=%d, height=%d, type=%c, keyframe=%d, fd=%d",
            hours, mins, secs, (100 * us) / AV_TIME_BASE, src_frame->width, src_frame->height, av_get_picture_type_char(src_frame->pict_type), src_frame->key_frame, framedrop_s);
        }
        else
        {
            snprintf(is->frame_info[is->pictq_windex], sizeof(is->frame_info[is->pictq_windex]), "pts=NaN, width=%d, height=%d, type=%c, keyframe=%d, fd=%d",
            src_frame->width, src_frame->height, av_get_picture_type_char(src_frame->pict_type), src_frame->key_frame, framedrop_s);
        }
    }
#endif
    ...
    {
        /* 자막 부분
        if (is->subtitle_st && is->subpq_size > 0)
        {
            int i;
            SubPicture *sp;
            sp = &is->subpq[is->subpq_rindex];
            if (vp->pts >= sp->pts + ((float) sp->sub.start_display_time / 1000))
                for (i=0; i<sp->sub.num_rects; i++)
                    blend_subrect((AVPicture*)src_frame, sp->sub.rects[i], src_frame->width, src_frame->height);
        }
        */
        SDL_UpdateYUVTexture(vp->bmp, NULL, src_frame->data[0], src_frame->linesize[0], src_frame->data[1], src_frame->linesize[1], src_frame->data[2], src_frame->linesize[2]);
    }
       ...
       /* NOTE: ipts is the PTS of the _first_ picture beginning in
        ...
        if (got_subtitle && sp->sub.format == 0) {
            ...
            for (i = 0; i < sp->sub.num_rects; i++)
            {
                ...
                int subw = is->viddec_width;
                int subh = is->viddec_height;
...
static int audio_decode_frame(VideoState *is)
{
    ...
    for (;;) {
        ...
        while (pkt_temp->stream_index != -1 || is->audio_buf_frames_pending) {
            ...
            //speed control
            wanted_nb_samples = synchronize_audio(is, is->frame->nb_samples / speed_s);
        ...
        if (pkt->data)
            av_free_packet(pkt);
        SDL_memset(pkt_temp, 0, sizeof(*pkt_temp));
        pkt_temp->stream_index = -1;
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while (len > 0) {
        ...
        if (len1 > len)
            len1 = len;
        SDL_MixAudio(stream, (uint8_t *)is->audio_buf + is->audio_buf_index, len1, volume_s); 
        len -= len1;
        stream += len1;
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        SDL_memset(&is->audio_pkt, 0, sizeof(is->audio_pkt));
        SDL_memset(&is->audio_pkt_temp, 0, sizeof(is->audio_pkt_temp));
...
static int read_thread(void *arg)
{
    ...
    SDL_memset(st_index, -1, sizeof(st_index));
    ...
    if (duration_hwnd != NULL && ic->duration != AV_NOPTS_VALUE)
    {
        ...
        int hours, mins, secs, us;
        int64_t duration_sd = ic->duration + 5000;
    ...
    if (!video_disable && !subtitle_disable)
        st_index[AVMEDIA_TYPE_SUBTITLE] =
            av_find_best_stream(ic, AVMEDIA_TYPE_SUBTITLE,
                                ...
                                NULL, 0);
    if (show_status) {
        av_dump_format(ic, 0, is->filename, 0, hide_msg);
    }
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    is->read_tid     = SDL_CreateThread(read_thread, "read_trhead", is);
    if (!is->read_tid) {
        av_free(is);
        return NULL;
    }
...
static void toggle_audio_display(VideoState *is)
{
    ...
    if (is->show_mode != next) {
        fill_rectangle(is->xleft, is->ytop, is->width, is->height, 0, 0, 0, 0);
        is->force_refresh = 1;
        is->show_mode = next;
    }
}
...
static void gui_message(int int_keycode)
{
#ifdef _WIN32
...
static void event_loop(VideoState *cur_stream)
{
    SDL_Event event;
    RECT s_rect;
    double incr, pos, pos1, pos2, frac;
    int dragging = 0;
            ...
            case SDL_KEYDOWN:
                ...
                switch (event.key.keysym.sym)
                {
                    ...
                    case SDLK_SPACE:
                        ...
                        break;
                    case SDLK_r:
                        s_resize_event(cur_stream, cur_stream->width, cur_stream->height);
                        break;
                    ...
                    do_seek:
                            if (seek_by_bytes) {
                                ...
                                pos1 = get_master_clock(cur_stream);
                                pos2 = (double)cur_stream->seek_pos / AV_TIME_BASE;
                                if (pos1 < 10) {
                                  if (!isnan(pos1)) {
                                    pos = FFMAX(pos1, pos2);
                                  } else {
            ...
            case SDL_WINDOWEVENT:
                switch (event.window.event)
                {
                    ...
                    case SDL_WINDOWEVENT_RESIZED:
                        if (is_move_s == 0) s_resize_event(cur_stream, event.window.data1, event.window.data2);
                        break;
                } 
                break; 
            case SDL_MOUSEWHEEL:
